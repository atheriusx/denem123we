...
module.exports = {
  name: "wl",
  description: "Ajoutez, supprimer, voir la liste de la whitelist",
  default_permission: true,
  options: [{
    name: 'add',
    description: "Ajoutez un utilisateur a la whitelist",
    type: 1,
    options: [{
      name: 'user',
    ...
    name: 'remove',
    description: "Supprimer un utilisateur de la whitelist",
    type: 1,
    options: [{
      name: "user",
      description: 'utilisateur',
    ...
    name: 'list',
    description: "Voir la list de la whitelist",
    type: 1,
  },
  ],
  timeout: 3000,
  ...
  run: async (client, interaction, args) => {
    ...
    if (subCommand === 'add') {
      ...
      if(!users) {
        ...
        interaction.reply({embeds: [new discord.MessageEmbed().setDescription(`Utilisateur déjà whitelist !`).setColor('RED')]})
      }
      ...
      return interaction.reply({
        embeds: [new discord.MessageEmbed()
          .setDescription(`L'utilisateur ${client.users.cache.get(user.id).tag} à été retirer de la whitelist`)
        ]
      })
